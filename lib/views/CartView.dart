...
class _CartViewState extends State<CartView> {
  ...
  Widget build(BuildContext context) {
    ...
    Column allWidgets = Column(children: [Container(
      width: w,
      child: Row(
        mainAxisAlignment:
    ...
    return Scaffold(
        appBar: AppBar(
          ...
          actions: [
           if(user.token != '' && user != null) Padding(
        ...
        body: user== null || user.token =='' ? Container(
            ...
            ? provideBusket != null && (provideBusket.success ?? false)
                      ...
                      Expanded(
                        child: ListView.builder(
                          itemBuilder: (context,i){
                            return i == 0 ? persons : i == (provideBusket.data.basket.length + 2) -1 ?  allWidgets :  ProductItem(i:i-1);
                          },
                            ...
                            onPressed: () {
                                showPlatformDialog(
                                  ...
                                  builder: (_) => BasicDialogAlert(
                                    title:  Text(provide.isOrdering ? 'Подождите' : "Вы уверены, что хотите cовершить покупку?",),
                                    actions: <Widget>[
                                      BasicDialogAction(
                                        title: Text("Отмена",style: TextStyle(
                                            color: Theme.of(context).primaryColor
                                        ),),
                                        onPressed: () {
                                      ...
                                      BasicDialogAction(
                                        title: Text("Да",style: TextStyle(
                                            color: Theme.of(context).primaryColor
                                        ),),
                                        onPressed: () {
                                          provide.order(user.token, context, samvyvoz,personsCount);
                                        },
                                      ),
                                    ],
                                  ),
                ...
                : provideBusket != null && (provideBusket.success ?? false) == false
                    ? Center(
...
class _ProductItemState extends State<ProductItem> {
  ...
  Widget build(BuildContext context) {
   var isDeleting = Provider.of<LoginProvide>(context).deletingItem;
    ...
    var items = Provider.of<LoginProvide>(context).provideBusketItems.data.basket;
   ...
   amount = items[widget.i].amount;
    ...
    final headerRow = Row(
        ...
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
          Expanded(flex: 3, child: Text( items[widget.i].name,)),
          Text('${amount *int.parse( items[widget.i].price,) } ₸')
        ]);
    final controlsRow = Row(
        ...
        children: [
          IconButton(
              onPressed: ()async{
                if (amount > 1) {
                  provider.changeCrement(false, items[widget.i].basketId,int.parse(items[widget.i].price),);
                  setState(() {
                    if (timer.isActive) timer.cancel();
                    timer = Timer(Duration(milliseconds: 600), () {
                      Provider.of<LoginProvide>(context,listen: false)
                          .changeValue(userToken, amount,  items[widget.i].basketId,);
                    });
                  });
                }else{
                  showPlatformDialog(
                    context: context,
                    builder: (_) => BasicDialogAlert(
                      title: Text("Вы уверены, что хотите удалить товар?",),
                      actions: <Widget>[
                        BasicDialogAction(
                          title: Text("Отмена",style: TextStyle(
                            color: Theme.of(context).primaryColor
                        ...
                        BasicDialogAction(
                          title: isDeleting != -1 ? LoadingCircle() : Text("Да",style: TextStyle(
                              color: Theme.of(context).primaryColor
                          ),),
                          onPressed: () {
                            Provider.of<LoginProvide>(context,listen: false).makeDelete( items[widget.i].basketId,);
                            setState(() {
                              amount = 0;
                              Provider.of<LoginProvide>(context,listen: false).deleteItem(userToken,  items[widget.i].basketId, context);
                            });
                          },
                        ),
                      ],
          ...
          IconButton(
              onPressed: () {
                provider.changeCrement(true, items[widget.i].basketId,int.parse(items[widget.i].price));
                setState(() {
                    if (timer.isActive) timer.cancel();
                    timer = Timer(Duration(milliseconds: 600), () {
                      Provider.of<LoginProvide>(context,listen: false)
