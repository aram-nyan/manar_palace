...
class _MainViewState extends State<MainView> {
  ...
  Widget build(BuildContext context) {
    ...
    var snapshot = Provider.of<ListViewProvider>(context,).categoriesModel;
    return snapshot != null && snapshot.length > 0 ? TabPage(
      content: ListView.builder(itemBuilder: (context,i){
        return AutoScrollTag(
          ...
          width: double.infinity,
                ...
                child: Text(provide.categoriesModel[i].categoryName, style: Theme.of(context).textTheme.headline2)
          ),
              )].followedBy(provide.categoriesModel[i].categories.products.map<Widget>((e) => Padding(
                padding: const EdgeInsets.all(8.0),
 ...
 Future<Widget> returnOneColumn(CategoriesModel categoriesModel)async{
  var header = await List.generate(categoriesModel.categories.products.length, (index){
    return ProductModel(
