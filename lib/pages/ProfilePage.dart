 ...
 static var id = 'profilePage';
  ...
  Widget build(BuildContext context) {
     var user = Provider.of<AuthProvide>(context).user;
      ...
      _ProfilePageEntryData('Служба поддержки', Icons.contact_phone, SupportPage.id),
      _ProfilePageEntryData('Обратный звонок', Icons.local_phone, FeedbackCallPage.id),
        if(user?.token != '' && user != null) _ProfilePageEntryData('Мои заказы', Icons.menu, HistoryOrder.id) ,
        if(user?.token != '' && user != null) _ProfilePageEntryData('Профиль', Icons.account_circle, ProfileDataScreen.id) ,
      _ProfilePageEntryData('Приложение Manar Palace', Icons.info, CompanyInfoPage.id),
        _ProfilePageEntryData('Наши заведения по Казахстану', Icons.add_location),
      _ProfilePageEntryData('Договоры оферты', Icons.insert_drive_file),
      _ProfilePageEntryData('Политика конфиденциальности', Icons.description),
    ];
    return AppPage(
      single: true,
      title: 'Кабинет',
      content: ListView(
        ...
        children: entries
          .map<Widget>((entry) => ListTile(
            leading: Icon(entry.icon, color: Color.fromRGBO(0x63, 0x63, 0x63, 1.0)),
          ...
          SizedBox(height: 16,),
          if(user?.token == '' && user != null)
          AppButton(label: 'Регистрация',
              onPressed: () => Navigator.of(context).pushNamed(RegisterPage.id)),
