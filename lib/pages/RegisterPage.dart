        ...
        content: Container(
            ...
            child: Form(
                child: Column(
                  ...
                  AppButton(
                      white: true,
                      enable: auth.registreMap['password'] ==
                              auth.registreMap['password_confirmation'] &&
                          auth.registreMap['password'].length > 5 &&
                          auth.registreMap['email'].contains('@'),
                      label: 'Регистрация',
                      trigger: auth.isLoading,
                      onPressed: () {
                        if (auth.registreMap['password'] ==
                                auth.registreMap['password_confirmation'] &&
                            auth.registreMap['password'].length > 5 &&
                            auth.registreMap['email'].contains('@')) {
                          auth.startLoading();
                          ...
                          auth.showLoseConnection(context, 'Вниание!',
                              'Проверьте корректность данных', true);
                        }
                      }),
                ]))));
  }
