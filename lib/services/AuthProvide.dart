...
class AuthProvide extends ChangeNotifier {
  ...
  BusketItems provideBusketItems =
      BusketItems(data: BusketStructure(basket: []));
  ...
  Future<dynamic> checkExist(context) async {
    try {
      final response =
          await http.post('https://manapolise.a-lux.dev/api/user/check', body: {
        'login': loginOrTel,
      });
  ...
  Future<dynamic> login(context) async {
    try {
      ...
      notifyListeners();
      final response =
          await http.post('https://manapolise.a-lux.dev/api/login', body: {
        'login': loginOrTel,
      ...
      showLoseConnection(
          context, 'Ошибка', 'Проверьте соединение с интернетом', true);
      print(e);
    }
    isLoading = false;
    notifyListeners();
  ...
  Future<dynamic> register(context) async {
    try {
      ...
      final response = await http
          .post('https://manapolise.a-lux.dev/api/register', body: registreMap);
      var jsonData = json.decode(response.body);
      print(jsonData);
      if (jsonData['success'] == true) {
        ...
        showLoseConnection(context, 'Ошибка', jsonData['data']['error'], true);
      }
    } catch (e) {
      print(e);
      showLoseConnection(
  ...
  Future<dynamic> sendEmail(context) async {
    try {
      ...
      final response = await http.post(
        'https://manapolise.a-lux.dev/api/user/password/code',
        body: {
          'email': forgotLogin,
        },
      );
  ...
  Future<dynamic> resetPassword(context) async {
    try {
      ...
      notifyListeners();
      final response = await http.post(
          'https://manapolise.a-lux.dev/api/user/password/reset',
          body: newPassword);
      var jsonData = json.decode(response.body);
      if (jsonData['success'] == true) {
  ...
  Future<dynamic> updateUser(context, token) async {
    try {
      ...
      notifyListeners();
      final response = await http.post(
        'https://manapolise.a-lux.dev/api/user/edit',
        body: newDataForUpdate,
        headers: {
